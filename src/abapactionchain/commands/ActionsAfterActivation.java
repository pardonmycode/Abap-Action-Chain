...
public class ActionsAfterActivation implements IActivationSuccessListener {
	...
	public void onActivationSuccess(List<IAdtObjectReference> adts, IProject arg1) {
		...
		try {
			for (Button btn : View.ButtonsList) {
				...
				for (IAdtObjectReference adt : adts) {
					System.out.println(this.getClass().getSimpleName());
					System.out.println(adt.getName());
					
					for (IAdtObjectReference adt_c : adt.getChildren()) {
						System.out.println(adt_c.getName());
	...
	public static void runTest() {
		...
		try {
			LegacyHandlerService srv = (LegacyHandlerService) ProjectUtility.service
					.executeCommand("com.sap.adt.tool.abap.unit.launchShortcut.run", null);
	...
	public static void runTestWithCoverage() {
		...
		try {
			ProjectUtility.service.executeCommand("com.sap.adt.tool.abap.unit.launchShortcutWithCoverage.coverage",
					null);
		} catch (ExecutionException | NotDefinedException | NotEnabledException | NotHandledException e) {
			e.printStackTrace();
		}
